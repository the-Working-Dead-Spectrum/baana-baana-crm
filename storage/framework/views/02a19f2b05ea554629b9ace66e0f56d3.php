...
<?php $__env->startSection('title', 'Logs de Synchronisation'); ?>
                ...
                <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        ...
                        <select name="type" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            ...
                            <option value="orders" <?php echo e(request('type') == 'orders' ? 'selected' : ''); ?>>Commandes</option>
                            <option value="products" <?php echo e(request('type') == 'products' ? 'selected' : ''); ?>>Produits</option>
                            <option value="creators" <?php echo e(request('type') == 'creators' ? 'selected' : ''); ?>>Créateurs</option>
                            <option value="all" <?php echo e(request('type') == 'all' ? 'selected' : ''); ?>>Tout</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            ...
                            <option value="success" <?php echo e(request('status') == 'success' ? 'selected' : ''); ?>>Succès</option>
                            <option value="failed" <?php echo e(request('status') == 'failed' ? 'selected' : ''); ?>>Échec</option>
                    ...
                    <div class="flex items-end">
                        ...
                        <?php if(request()->anyFilled(['type', 'status', 'start_date'])): ?>
                            <a href="<?php echo e(route('admin.sync.logs')); ?>" 
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">
                                                <?php echo e($log->started_at->format('d/m/Y')); ?>

                                            </div>
                                            <div class="text-xs text-gray-500">
                                                <?php echo e($log->started_at->format('H:i:s')); ?>

                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                                <?php echo e($log->sync_type === 'orders' ? 'bg-blue-100 text-blue-800' : 
                                                   ($log->sync_type === 'products' ? 'bg-purple-100 text-purple-800' : 
                                                   ($log->sync_type === 'creators' ? 'bg-green-100 text-green-800' : 
                                                   'bg-gray-100 text-gray-800'))); ?>">
                                                <?php echo e(ucfirst($log->sync_type)); ?>

                                            </span>
                                            <?php if(isset($log->metadata['trigger']) && $log->metadata['trigger'] === 'manual'): ?>
                                                <span class="ml-1 text-xs text-yellow-600" title="Sync manuelle">👤</span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                                <?php echo e($log->status === 'success' ? 'bg-green-100 text-green-800' : 
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">
                                                <?php echo e(number_format($log->total_records)); ?>

                                            </div>
                                            <?php if($log->failed_records > 0): ?>
                                                <div class="text-xs text-red-500">
                                                    <?php echo e($log->failed_records); ?> échecs
                                                </div>
                                            <?php endif; ?>
                                        </td>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\wp-cs-crm\resources\views/admin/sync/logs.blade.php ENDPATH**/ ?>