    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center">
                <div>
                    <a href="{{ route('creator.products') }}" class="text-blue-600 hover:text-blue-800 text-sm mb-2 inline-flex items-center">
                    ...
                    <p class="text-gray-600 mt-2">{{ $creator->brand_name }}</p>
                </div>
                <div>
                    @if($salesStats && ($salesStats->total_quantity ?? 0) > 0)
                            ...
                            <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-6 border border-green-200">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-3xl font-bold text-green-900">{{ number_format($salesStats->total_sales ?? 0, 0, ',', ' ') }} CFA</p>
                                    </div>
                            ...
                            <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-6 border border-blue-200">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-3xl font-bold text-blue-900">{{ $salesStats->total_quantity ?? 0 }}</p>
                                    </div>
                            ...
                            <div class="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-6 border border-purple-200">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-3xl font-bold text-purple-900">{{ $salesStats->order_count ?? 0 }}</p>
                                    </div>
                            ...
                            <div class="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg p-6 border border-yellow-200">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-3xl font-bold text-yellow-900">{{ number_format($salesStats->average_unit_price ?? 0, 0, ',', ' ') }} CFA</p>
                                    </div>
                                                ...
                                                <tr class="hover:bg-gray-50 transition">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                            {{ $productItem->quantity }} unité{{ $productItem->quantity > 1 ? 's' : '' }}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ $order->status == 'completed' ? 'green' : ($order->status == 'pending' ? 'yellow' : 'gray') }}-100 text-{{ $order->status == 'completed' ? 'green' : ($order->status == 'pending' ? 'yellow' : 'gray') }}-800">
                                                            {{ ucfirst($order->status) }}
                                                        </span>
                                                    </td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                        <a href="{{ route('creator.orders.show', $order->id) }}" class="text-blue-600 hover:text-blue-900">
                                                            Voir →
                                                        </a>
                                                    </td>
                                                </tr>
            ...
            <div class="space-y-6">
                    ...
                    <div class="p-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="text-lg font-bold text-gray-900">{{ number_format($product->price, 0, ',', ' ') }} CFA</p>
                            </div>
                            ...
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-900">{{ $product->wp_product_id }}</p>
                            </div>

                            <div>
                                <p class="text-xs text-gray-500 mb-1">Marque</p>
                                <p class="text-sm font-medium text-gray-900">{{ $product->brand_slug }}</p>
                            </div>

                            <div>
                                <p class="text-xs text-gray-500 mb-1">Date d'ajout</p>
                                <p class="text-sm text-gray-900">{{ $product->created_at?->format('d/m/Y') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="space-y-3">
                            <div class="flex items-start">
                                ...
                                <p class="ml-3 text-sm text-indigo-900">
                                    Vendu dans <span class="font-semibold">{{ $salesStats->order_count }}</span> commande{{ $salesStats->order_count > 1 ? 's' : '' }}
                                </p>
                            </div>

                            @if($salesStats->total_quantity > 0)
                            <div class="flex items-start">
                                <div class="flex-shrink-0">
                                    <svg class="h-5 w-5 text-indigo-600" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                ...
                                <p class="ml-3 text-sm text-indigo-900">
                                    Moyenne de <span class="font-semibold">{{ round($salesStats->total_quantity / $salesStats->order_count, 1) }}</span> unité{{ ($salesStats->total_quantity / $salesStats->order_count) > 1 ? 's' : '' }} par commande
                                </p>
                            </div>
                            @endif

                            @if($salesStats->total_sales > 0)
                            <div class="flex items-start">
                                <div class="flex-shrink-0">
                                    <svg class="h-5 w-5 text-indigo-600" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                ...
                                <p class="ml-3 text-sm text-indigo-900">
                                    Génère en moyenne <span class="font-semibold">{{ number_format($salesStats->total_sales / $salesStats->order_count, 0, ',', ' ') }} CFA</span> par commande
                                </p>
                            </div>
                            @endif
                        </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <div class="space-y-2">
                            <a href="{{ route('creator.products') }}" class="block w-full text-center px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition">
                            ...
                            @if($recentOrders && $recentOrders->count() > 0)
                            <a href="{{ route('creator.orders', ['search' => $product->name]) }}" class="block w-full text-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition">
                                <svg class="inline-block w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                                </svg>
                                Voir les commandes
