                    ...
                    @if (isset($allBrands) && count($allBrands) > 0)
                        <div class="mb-4">
                            <div class="text-sm text-gray-600">
                                Total : <span class="font-semibold">{{ count($allBrands) }}</span> marques |
                                Produits avec marque : <span
                                    class="font-semibold">{{ $summary['total_products_with_brand'] ?? 0 }}</span> |
                                Produits sans marque : <span
                                    class="font-semibold">{{ $summary['total_products_without_brand'] ?? 0 }}</span>
                            </div>
                        </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($allBrands as $brandSlug)
                                        ...
                                        <tr class="hover:bg-gray-50 {{ !$hasProducts ? 'bg-gray-50' : '' }}">
                                            <!-- Marque -->
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-center">
                                                    <span
                                                        class="text-lg font-semibold {{ $hasProducts ? 'text-gray-900' : 'text-gray-400' }}">
                                                        {{ $productCount }}
                                                    </span>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-center">
                                                    @if (($brandData['total_quantity'] ?? 0) > 0)
                                                        <div class="text-sm font-medium text-blue-600">
                                                            {{ number_format($brandData['total_quantity'], 0, ',', ' ') }}
                                                        </div>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                @if ($hasProducts)
                                                    <a href="{{ route('products.by-brand', $brandSlug) }}"
                    ...
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="text-sm text-gray-500">
                                {{ $allProducts->total() }} produit(s) au total
                            </div>
                            <div class="text-xs text-gray-400">
                                {{ $summary['products_with_sales'] ?? 0 }} avec ventes
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-6 bg-gray-50 p-4 rounded-lg">
                        <form method="GET" action="{{ route('admin.products') }}" class="space-y-4">
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="search" id="search"
                                        value="{{ $filters['search'] ?? '' }}" placeholder="Nom, SKU, ID..."
                                ...
                                <div>
                                    ...
                                    <select name="brand" id="brand"
                                        ...
                                        @foreach ($allBrands as $brandSlug)
                                            <option value="{{ $brandSlug }}"
                                                {{ ($filters['brand'] ?? '') === $brandSlug ? 'selected' : '' }}>
                                                {{ $brandSlug }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select name="has_sales" id="has_sales"
                                        ...
                                        <option value="yes"
                                            {{ ($filters['has_sales'] ?? '') === 'yes' ? 'selected' : '' }}>Avec ventes
                                        </option>
                                        <option value="no"
                                            {{ ($filters['has_sales'] ?? '') === 'no' ? 'selected' : '' }}>Sans vente
                                        </option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="flex space-x-2">
                                        <select name="sort_by" id="sort_by"
                                            ...
                                            <option value="name"
                                                {{ ($filters['sort_by'] ?? 'name') === 'name' ? 'selected' : '' }}>Nom
                                            </option>
                                            <option value="price"
                                                {{ ($filters['sort_by'] ?? '') === 'price' ? 'selected' : '' }}>Prix
                                            </option>
                                            <option value="brand"
                                                {{ ($filters['sort_by'] ?? '') === 'brand' ? 'selected' : '' }}>Marque
                                            </option>
                                            <option value="sales"
                                                {{ ($filters['sort_by'] ?? '') === 'sales' ? 'selected' : '' }}>CA</option>
                                            <option value="quantity"
                                                {{ ($filters['sort_by'] ?? '') === 'quantity' ? 'selected' : '' }}>Quantité
                                            </option>
                                        </select>
                                        <select name="sort_order"
                                            class="w-24 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                                            <option value="asc"
                                                {{ ($filters['sort_order'] ?? 'asc') === 'asc' ? 'selected' : '' }}>↑ Asc
                                            </option>
                                            <option value="desc"
                                                {{ ($filters['sort_order'] ?? '') === 'desc' ? 'selected' : '' }}>↓ Desc
                                            </option>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                @if ($filters['search'] || $filters['brand'] || $filters['has_sales'])
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($products as $product)
                                        <tr
                                            class="hover:bg-gray-50 {{ $product['has_sales'] ? '' : 'bg-gray-50 opacity-75' }}">
                                            <!-- Nom du produit -->
                                            <td class="px-6 py-4">
                                                <div class="flex items-center">
                                                    <div>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                ...
                                                @if (
                                                    ...
                                                    <div class="text-xs text-gray-500">
                                                        Moy:
                                                        {{ number_format($product['average_price_per_unit'], 0, ',', ' ') }}
                                                        CFA
                                                    </div>
                                                @endif
                                            </td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                @if ($product['real_id'])
                                                    <a href="{{ route('products.show', $product['real_id']) }}"
