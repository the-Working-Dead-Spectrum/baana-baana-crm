        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-semibold text-gray-900">{{ number_format($syncStats['total_orders']) }}</p>
                            <p class="text-xs text-gray-500 mt-1">{{ $syncStats['today_orders'] }} aujourd'hui</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-semibold text-gray-900">{{ number_format($syncStats['total_creators']) }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-semibold text-gray-900">{{ number_format($syncStats['total_products']) }}</p>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-lg font-semibold text-gray-900">
                                @if ($syncStats['last_sync'])
                                    {{ $syncStats['last_sync']->started_at->diffForHumans() }}
                                @else
                                    Jamais
                                @endif
                            </p>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-8">
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('admin.sync.run') }}" id="sync-form" class="space-y-4">
                        @csrf
            ...
            <!-- Queue Status -->
            @if (isset($syncStats['queue_status']['warning']))
                <div class="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded mb-8">
                    <strong>⚠️ Attention:</strong> {{ $syncStats['queue_status']['warning'] }}
                </div>
            @endif
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            <p class="text-2xl font-semibold {{ $syncStats['queue_status']['pending'] > 0 ? 'text-yellow-600' : 'text-gray-900' }}">
                                {{ $syncStats['queue_status']['pending'] ?? 0 }}
                            </p>
                        </div>
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <p class="text-sm font-medium text-gray-500">Jobs échoués</p>
                            <p class="text-2xl font-semibold {{ ($syncStats['queue_status']['failed'] ?? 0) > 0 ? 'text-red-600' : 'text-gray-900' }}">
                                {{ $syncStats['queue_status']['failed'] ?? 0 }}
                            </p>
                        </div>
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <p class="text-sm font-medium text-gray-500">Dernier échec</p>
                            <p class="text-lg font-semibold text-gray-900">
                                @if (isset($syncStats['queue_status']['last_failed']))
                                    {{ \Carbon\Carbon::parse($syncStats['queue_status']['last_failed']->failed_at)->diffForHumans() }}
                                @else
                                    Jamais
                                @endif
                            </p>
